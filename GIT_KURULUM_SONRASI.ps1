# Git Kurulum Sonrası Otomatik Push Script
# Git kurulumunu tamamladıktan sonra bu script'i çalıştırın

Write-Host ""
Write-Host "═══════════════════════════════════════════════════════" -ForegroundColor Cyan
Write-Host "   🚀 GIT KURULUM SONRASI OTOMATIK PUSH" -ForegroundColor Cyan
Write-Host "═══════════════════════════════════════════════════════" -ForegroundColor Cyan
Write-Host ""
...
try {
    ...
    if ($LASTEXITCODE -eq 0) {
        ...
        Write-Host ""
        Write-Host "⚠️  Lütfen önce Git'i kurun:" -ForegroundColor Yellow
        Write-Host "   1. https://git-scm.com/download/win adresinden indirin" -ForegroundColor Cyan
        Write-Host "   2. Kurun ve PowerShell'i yeniden başlatın" -ForegroundColor Cyan
        Write-Host "   3. Bu script'i tekrar çalıştırın" -ForegroundColor Cyan
        pause
        exit
    }
} catch {
...
$commitMessage = "Update email to info@ynadijital.com, remove phone numbers, add leads management and improvements"
...
if ($pushResult -eq 0) {
    ...
    Write-Host ""
    Write-Host "🌐 Site URL: https://turkeyclinicfinder.vercel.app" -ForegroundColor Cyan
} else {
    Write-Host ""
